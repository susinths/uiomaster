...
IRQconfig=$(echo  -n $(awk '/.*virtio3-config$/ {print $4}  ' /proc/interrupts)) 
IRQinput=$(echo  -n $(awk '/.*virtio3-input.0$/ {print $5}  ' /proc/interrupts))
IRQoutput=$(echo  -n $(awk '/.*virtio3-output.0$/ {print $3}  ' /proc/interrupts))
